...
export class VolumesService {
  ...
  async createVolume(dto: CreateVolumeDto, file? : Express.Multer.File) {
    ...
    try {
      ...
      if(file){
        const cloudData = await this.cloudService.uploadImage(file, 'story_vault/volumes');
    ...
    async getAllVolumes(cursorId?: string, searchKeyword?: string, activeStatus?: string) {
  ...
  async updateVolume(volumeId: string, updateVolumeDto: UpdateVolumeDto, file? : Express.Multer.File) {
    try {
