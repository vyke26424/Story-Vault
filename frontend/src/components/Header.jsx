...
const Header = () => {
  ...
  const handleLogout = async () => {
    try {
      // Gọi API báo cho Server xóa RefreshToken trong DB và xóa Cookie
      await axiosClient.post('/auth/logout');
    } catch (error) {
      console.error("Lỗi khi đăng xuất backend:", error);
    } finally {
      // Dù API có lỗi hay không, Frontend vẫn phải xóa sạch data user
      logout();
      navigate('/');
    }
  };
        ...
        <div className="flex flex-wrap md:flex-nowrap justify-between items-center gap-4">
          ...
          <div className="w-full md:w-auto flex-1 max-w-xl hidden sm:block">
            <div className="relative">
              ...
              />
              <button onClick={handleSearch} className="absolute right-3 top-1/2 -translate-y-1/2 text-amber-700 hover:text-amber-900 font-bold">
                🔍
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 shrink-0">
            <nav className="space-x-6 hidden lg:flex font-bold text-stone-600 text-sm">
              ...
              <Link to="/category" className="hover:text-amber-700 transition-all">Danh mục</Link>
            </nav>
            ...
            {isAuthenticated && user ? (
              ...
              <div className="hidden md:flex items-center gap-4">
                <div className="flex flex-col items-end">
                  <span className="text-sm font-bold text-stone-800">{user.name || user.email}</span>
                  <span className="text-[10px] uppercase tracking-wider font-black text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full">
                    {user.role}
                  </span>
                </div>
                <button 
                  onClick={handleLogout}
                  className="text-stone-500 hover:text-red-600 font-bold text-sm transition-colors"
                >
                  Đăng xuất
                </button>
              </div>
